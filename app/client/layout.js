...
import {
  ...
  LayoutDashboard, Globe, Wrench, FileText
} from 'lucide-react'
...
export default function ClientLayout({ children }) {
  ...
  useEffect(() => {
    ...
    if (!roleCookie || roleCookie !== 'client') {
      showAlert({
        icon: 'warning',
  ...
  const goTo = (path) => router.push(`/client/${path}`)
  ...
  const menuItems = [
    ...
    { icon: User, label: texts.profile, path: 'profile' },
    { icon: Wrench, label: texts.services, path: 'services' },
    { icon: FileText, label: texts.history, path: 'history' },
  ]

  const isActive = (path) => {
    if (pathname === '/client' && path === '') return true
    return pathname === `/client/${path}`
  }
          ...
          {sidebarOpen ? (
            <button
              ...
              className={`
                ...
                ${darkMode ? "hover:bg-slate-700/40" : "hover:bg-blue-50"}
              `}
            >
